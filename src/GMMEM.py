...
class GMMEM():
    ...
    def e_step(self, X):
        ...
        log_r = np.log(gmm_pdf) - np.log(np.sum(gmm_pdf, 1, keepdims=True) + self.eps) # (N, K)
    ...
    def m_step(self, X):
        ...
        self.sigma = ((r_tile * res_error) @ res_error.transpose(0, 2, 1)) / (N_k[:,None,None] + self.eps) # (K, D, D)
